...
import { Brain, Clock, Sparkles, TrendingDown, Plus, BookOpen, FileText, Image, FileSpreadsheet } from "lucide-react";
      ...
      <div className="flex-1 grid grid-cols-1 lg:grid-cols-3 gap-6 px-6 pb-6 overflow-hidden">
        ...
        <Card className="border-border/50 flex flex-col overflow-hidden">
          <CardHeader className="flex-shrink-0">
            ...
            <CardDescription>Click to view uploaded files</CardDescription>
          </CardHeader>
          <CardContent className="flex-1 overflow-hidden p-0">
            <ScrollArea className="h-full px-6 pb-6">
              {papersLoading ? (
                ...
                <div className="space-y-3">
                  {papers.map((paper) => (
                    <div
                      ...
                      className="p-4 rounded-lg border border-border/50 bg-muted/30 hover:border-primary/50 hover:bg-primary/5 transition-colors cursor-pointer"
                    >
                      ...
                      <div className="flex items-center gap-4 mt-3 text-xs text-muted-foreground">
                        ...
                        {paper.file_type && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded bg-accent/20 text-accent-foreground">
                            {paper.file_type.split('/')[1]?.toUpperCase() || 'FILE'}
                          </span>
                        )}
                      </div>
                    </div>
